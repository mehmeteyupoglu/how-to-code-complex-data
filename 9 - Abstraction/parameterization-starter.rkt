...
(define (positive-only lon) (neg-or-pos positive? lon))
...
(define (negative-only lon) (neg-or-pos negative? lon))


(define (neg-or-pos type lon)
  (cond [(empty? lon) empty]
        [else
         (if (type (first lon))
             (cons (first lon)
                   (neg-or-pos type (rest lon)))
             (neg-or-pos type (rest lon)))]))
